...
class DataAggregationTask:
    ...
    def write_data_to_file(self, data: dict) -> None:
        with open('result.json', 'w', encoding='utf-8') as file:
            json.dump(self._get_renamed_dict(data), file,  ensure_ascii=False, indent=2)
...
class DataAnalyzingTask:
    ...
    def get_best_cities(self, data):
        result = ''.join([key for key, value in data.items() if value['rating'] == 1])
        cities = 'Лучший город' if len(result) == 1 else 'Лучшие города'
        return f'{cities} для отдыха это: {result}'
...
if __name__ == '__main__':
    ...
    obj_agr.write_data_to_file(calc_result)
    print(DataAnalyzingTask().get_best_cities(calc_result))
