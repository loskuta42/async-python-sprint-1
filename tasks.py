...
from utils import BAD_CONDITIONS, CITIES_RUS
...
class DataCalculationTask:
    ...
    def _get_params_for_city(data: dict) -> dict:
        try:
            ...
            conditions = [date_data['cond_points'] for date_data in data.values() if date_data['cond_points']]
            mean_cond = round(mean(conditions), 1)
            total_score = mean_temp + mean_cond
            return {
                'avg_temp_for_city': mean_temp,
                'avg_cond_for_city': mean_cond,
            ...
            result = {
                date:
                    (self._calculate_data_for_date(date_data)
                     if date_data
                     else {'avg_temp': None, 'cond_points': None}
                     )
